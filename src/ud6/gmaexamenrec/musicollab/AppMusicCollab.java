...
public class AppMusicCollab {
    ...
    public static List<Musico> musicosParaCancion(Cancion c, List<Musico> musicos) {
        List<Musico> musicosCancion = new ArrayList<>();

        for (Musico m : musicos) {
            if (m.getInstrumentosQueDomina().contains(c.getInstrumentosRequeridos())) {
                musicosCancion.add(m);
            }
        }
        return musicosCancion;
    }

    public static List<Cancion> cancionesPosibles(List<Musico> grupo, List<Cancion> canciones) {
        List<Cancion> cancionPosible = new ArrayList<>();
        for (Musico m : grupo) {
            for (Cancion c : canciones) {
                if (m.getInstrumentosQueDomina().containsAll(c.getInstrumentosRequeridos())) {
                    cancionPosible.add(c);
                }
            }
        }
        cancionPosible.sort((c1, c2) -> c1.getNombre().compareTo(c2.getNombre()));
        return cancionPosible;
    }
    ...
    public static void main(String[] args) {
        ...
        List<Musico> musicos = new ArrayList<>(Arrays.asList(
                ...
                new Musico("Carlos Ruiz", "GuitarHero", List.of("guitarra", "voz"),
                        new HashSet<>(Arrays.asList(canciones.get(0), canciones.get(4), canciones.get(7)))),
                new Musico("Luisa Pérez", "DrumStar", List.of("batería"),
                        new HashSet<>(Arrays.asList(canciones.get(1), canciones.get(3), canciones.get(6)))),
                new Musico("Pablo Martín", "PianoMaster", List.of("piano"),
                        new HashSet<>(Arrays.asList(canciones.get(0), canciones.get(2), canciones.get(6)))),
                new Musico("Sofía Vega", "Vocalist", List.of("voz"),
                        new HashSet<>(Arrays.asList(canciones.get(2), canciones.get(4), canciones.get(7)))),
                ...
                new Musico("Elena Díaz", "MultiInstrumentalist", List.of("guitarra", "piano", "voz"),
                        new HashSet<>(Arrays.asList(canciones.get(0), canciones.get(2), canciones.get(7))))));
